...
public:
    ...
    static void CopyNativeImage(NativeImage *pSrcImg, NativeImage *pDstImg) {
        if (pSrcImg == nullptr || pSrcImg->ppPlane[0] == nullptr) {
            LOGE("NativeImageUtil::CopyNativeImage  error pSrcImg == nullptr || pSrcImg->ppPlane[0] == nullptr");
            return;
        }
        ...
        if (pSrcImg->format != pDstImg->format ||
            ...
            LOGE("NativeImageUtil::CopyNativeImage  error");
            return;
        }
        if (pDstImg->ppPlane[0] == nullptr) {
            LOGE("NativeImageUtil::CopyNativeImage  pDstImg->ppPlane[0] == nullptr  AllocNativeImage");
        ...
        switch (pSrcImg->format) {
            ...
            case IMAGE_FORMAT_NV12: {
                ...
                break;
            case IMAGE_FORMAT_RGBA: {
                LOGE("NativeImageUtil::CopyNativeImage  IMAGE_FORMAT_RGBA memcpy ");
                memcpy(pDstImg->ppPlane[0], pSrcImg->ppPlane[0],
